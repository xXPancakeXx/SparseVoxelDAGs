...
{
	[BurstCompile]
    public struct VoxelizeColorJob : IJob
    {
		...
		public UnsafeList<UnsafeLongList<ColorOctNode>> levels;

		public VoxelizeColorJob(NativeLocalMesh mesh, int maxDepth, VoxelTreeLevels<ColorOctNode> voxelLevels)
        {
            this.mesh = mesh;
        ...
        public unsafe void Execute()
        {
			...
			//Init root voxel
			levels.ElementAt(0).Add(new ColorOctNode());
                    ...
                    for (int i = 7; i >= 0; --i)
                    {
                        if (IntersectTriBox(childrenCenters[i], k, tri))
                        {
                            ...
                            if (!node.HasChild(i))
                            {
                                ...
                                //                       {
                                var col = mesh.GetColor(triIdx, tri, (float3)childrenCenters[i]);
								node.colors[i] = *(int*)&col;
                                //                       }
                                ...
                                if (qi.level < maxDepth - 1)
                                {
									...
									levels.ElementAt(qi.level + 1).Add(new ColorOctNode());              //create node at last element in next level
								}
