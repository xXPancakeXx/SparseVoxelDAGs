...
{
	[BurstCompile]
    public struct VoxelizeTriJob : IJob
    {
		...
		public VoxelizeTriJob(UnsafeList<Tri> triangles, Bounds triBounds, int maxDepth, VoxelTreeLevels<OctNode> voxelLevels)
        {
            this.triangles = triangles;
            this.bounds = triBounds;
        ...
        public unsafe void Execute()
        {
            ...
            for (int triIdx = 0; triIdx < triangles.Length; triIdx++)
            {
                ...
                //{
                //    Debug.Log($"{(triIdx / (float)triangles.Length * 100)}%");
                //}
                    ...
                    for (int i = 7; i >= 0; --i)
                    {
                        if (IntersectTriBox(childrenCenters[i], k, tri))
                        {
                            ...
                            if (!node.HasChild(i))
                            {
                                ...
                                nNodes++;
                                //levels.ElementAt(qi.level)[qi.nodeIdx] = node;
                            }
