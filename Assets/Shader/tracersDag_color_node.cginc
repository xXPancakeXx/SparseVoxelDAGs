...
inline bool IsPenultimateLevel(const int scale, const int maxDepth)
{
    //maxscale = 23
    //maxdepth = 2
    //minscale = 21
...
inline uint SumPrecedingChildrenSubtreeCount(StructuredBuffer<uint> data, const uint parentIndex, const int node, int childBit, bool isLeafNode, bool isPenultimateNode)
{
    if (isLeafNode)
    {
        int lowerIndexLeafOctantMask = childBit - 1;
    ...
    while ((childBit >>= 1) != 0)
    {
        int hasChild = (childBit & node);
        if (hasChild)
        {
            uint childIndex = FetchChildIndex(data, parentIndex, node, childBit);

            int childSubtreeCount;
            if (!isPenultimateNode)         //for any level except the penultimate we stored the subtree node count at index    
            {
                childSubtreeCount = data[childIndex + 1];
            }
            else
            {
                int childValidMask = data[childIndex];
                int lowerIndexLeafOctantMask = childBit - 1;
                childSubtreeCount = countbits(childValidMask) + 1;
            }

            sum += childSubtreeCount;
...
void TraceDag(inout StructuredBuffer<uint> _octreeNodes, int maxDepth, Ray ray, out TraceResult res, float projFactor)
{
    ...
    while (scale < MAX_SCALE)
    {
        ...
        if (isValidNode != 0 && minT <= maxT)
        {
            ...
            //if (minT * scaleExp2 > projFactor)
            if (minT * projFactor > scaleExp2)
            {
                res.t = minT;
                res.colorIndex = attributeIndex;
                // res.colorIndex = MAX_SCALE - scale;
            ...
            if (minT <= maxTV)
            {
                ...
                int attOffset = SumPrecedingChildrenSubtreeCount(_octreeNodes, parentIndex, currentNode, childBit, isLeafNode, IsPenultimateLevel(scale, maxDepth)) + 1; //+1 for the node itsself
