...
inline uint FetchLeafValidMask(RWStructuredBuffer<SVONode> _octreeNodes, const uint parentIndex, const SVONode node, const int childBit)
{
    int lowerIndexLeafOctantMask = childBit - 1;
    int childIndexOffset = countbits((uint) (GetValidMask(node) & lowerIndexLeafOctantMask));

    int idx = parentIndex + node.childrenIndex / 8;
    int nodeOffset = node.childrenIndex % 8 + childIndexOffset;

    SVONode node = _octreeNodes[idx];
    int validMask = 0;
