...
class Voxelizer {
    ...
    void Voxelize() {
        ...
        auto childrenCenters = stackalloc double3[8];

        auto triCount = mesh.triangles.Length / 3;
        for (int triIdx = 0; triIdx < triCount; triIdx++)
        {
            ...
            auto tri = mesh.GetTriangle(triIdx);
            ...
            while (queue.Count > 0)
            {
                ...
                ref auto node = ref levels.ElementAt(qi.level).ElementAt(qi.nodeIdx);
                for (int i = 7; i >= 0; --i)
                {
	...
	private bool IntersectTriBox(double3 boxCenter, double boxHalfSide, Tri tri)
	{
